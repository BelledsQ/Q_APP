...
@synthesize tit,des,oldtit,olddes,soldtit,solddes,sceneID,scenetype,wanip;
...
- (void)viewDidLoad
{
    ...
    if([scenetype isEqualToString:@"group"]){
        self.navigationItem.title = [NSString stringWithFormat:@"MOD Group"];
    }else{
...
-(NSString *)NSString2Json:(int)value
{
    ...
    switch (value) {
        case 1:
            //获取组列表
            cmdctrl = [[NSMutableDictionary alloc] initWithObjectsAndKeys:@"set_group_title",@"cmd",tit.text,@"group_title",des.text,@"description",sceneID,@"group_id",nil];
            break;
