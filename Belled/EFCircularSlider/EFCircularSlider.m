...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        angle = 0;
        radius = self.frame.size.height/2 - _lineWidth/2 - 30;
...
-(void) drawHandle:(CGContextRef)ctx{
    ...
    }else if(_handleType == nullBigCircle) {
        [_handleColor set];
        CGContextFillEllipseInRect(ctx, CGRectMake(handleCenter.x-15, handleCenter.y-15, _lineWidth+30, _lineWidth+30));
    }else if(_handleType == nullCircle) {
        [_handleColor set];
        CGContextFillEllipseInRect(ctx, CGRectMake(handleCenter.x-0, handleCenter.y-0, _lineWidth+0, _lineWidth+0));
    }
